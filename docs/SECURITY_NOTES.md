...
hex characters and is decoded to `32..256` bytes before key derivation:
...
also be generated by the OS CSPRNG. The library decodes it to `32..256` secret
...
The pepper file must contain `64..512` hex characters. The library decodes it to
`32..256` pepper bytes before key derivation. The raw pepper file is capped at
512 bytes, so a max-length pepper must not include a trailing newline. The
default path is:
