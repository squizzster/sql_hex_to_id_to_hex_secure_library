...
is public and is rejected during normal library use. Replace it with a
deployment-specific random hex value. The source value must contain `64..512`
hex characters. The decoded `32..256` bytes are validated and normalized before
key derivation:
...
The demo and tests set `XCTX_DEMO_ALLOW_BUNDLED_DOMAIN_SALT=1` so sample IDs can
still be generated with the bundled salt. Do not set that environment variable
for deployed application processes.

`XCTX_ID_PASSWORD` must be a separate `64..512` character hex value and should
