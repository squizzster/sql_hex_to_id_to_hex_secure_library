...
`DOMAIN_SALT_HEX` is a public-ID deployment hardening value near the top of
`sql_id_library.py`. The bundled salt is public and accepted by the library, but
deployed applications should replace it with a deployment-specific 32-byte
random hex value:
...
If an attacker does not have the deployment salt, they need both the deployed
source/config value and `XCTX_ID_PASSWORD` to reproduce the public-ID scheme.
Do not treat the salt as a substitute for the runtime secret.
