...
deployment-specific 32-byte random hex value. The source value must be exactly
`64` hex characters and is decoded to `32` bytes before key derivation:
...
`XCTX_ID_PASSWORD` must be a separate `64..256` character hex value and should
also be generated by the OS CSPRNG. The library decodes it to `32..128` secret
...
The pepper file must contain `64..256` hex characters. The library decodes it to
`32..128` pepper bytes before key derivation. The default path is:
