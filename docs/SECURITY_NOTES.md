...
- `SQL_ID_LIBRARY_DOMAIN_SALT_HEX` from the process environment
- `SQL_ID_LIBRARY_PASSWORD_HEX` from the process environment
- the disk pepper file configured by `pepper_file_location`

The domain salt is a public-ID deployment hardening value. Set
`SQL_ID_LIBRARY_DOMAIN_SALT_HEX` to a deployment-specific random hex value. The
library does not contain a deployment salt fallback. The active value must
...
`SQL_ID_LIBRARY_PASSWORD_HEX` must be a separate `64..512` character hex value and should
